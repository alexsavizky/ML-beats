...
def generate_lofi_music(model, seed, seq_length=128, generate_length=500):
    ...
    for _ in range(generate_length // seq_length):
        ...
        pred = model.predict(np.expand_dims(input_seq, axis=0))
...
X, y = data_gen[0]
print(X.shape, y.shape)  # Ensure X and y shapes are consistent
...
seed = X[0]  # Use the first sequence in X as the seed
